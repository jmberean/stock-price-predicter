...
def plot_results_with_forecast(dates, actual_prices, future_pred_inv, future_dates):

    # Ensure data is formatted properly
    dates = pd.to_datetime(dates)
    future_dates = pd.to_datetime(future_dates)
    
    # Flatten the actual_prices array to 1D
...
if __name__ == "__main__":
    ...
    # Plot results using actual close prices
    actual_prices = df['Close'].values
    plot_results_with_forecast(df.index, actual_prices, future_pred_inv, future_dates)
